...
class Builder extends BaseCommand
{
    ...
    public function __construct(?string $name = null)
    {
        ...
        $translatable_factory = new TranslatableFactory(
            $domain_options,
            new Translator($domain_options, $locale_options, $symfony_translator_proxy)
        );
        ...
        $file_syncer = new FileSyncer(
            $environment,
            $finder,
            $file_system,
            $this->pathFactory,
            $path_list_factory,
            $rsync_process_runner,
        ...
        $active_dir_is_ready = new ActiveDirIsReady(
            $environment,
            $active_dir_exists,
            $active_dir_is_writable,
            $translatable_factory
        );
        ...
        $composer_available = new ComposerIsAvailable(
            $composer_process_runner,
            $environment,
            $finder,
            $output_callback,
            $translatable_factory
        );
        $host_supports_running = new HostSupportsRunningProcesses(
            $environment,
            $process_factory,
            $translatable_factory
        );
        ...
        $common_preconditions = new CommonPreconditions(
            $environment,
            $translatable_factory,
            $active_and_staging,
            $active_dir_is_ready,
            $composer_available,
            $host_supports_running,
            $no_nesting_on_windows,
            $rsync_is_available
        );
        $file_finder = new FileFinder($this->pathFactory, $path_helper, $path_list_factory, $translatable_factory);
        $no_abs_sym = new NoAbsoluteSymlinksExist(
            $environment,
            $file_finder,
            $file_system,
            $this->pathFactory,
        ...
        $no_hard_links = new NoHardLinksExist(
            $environment,
            $file_finder,
            $file_system,
            $this->pathFactory,
        ...
        $no_links_windows = new NoLinksExistOnWindows(
            $environment,
            $file_finder,
            $file_system,
            $this->pathFactory,
        ...
        $no_sym_outside = new NoSymlinksPointOutsideTheCodebase(
            $environment,
            $file_finder,
            $file_system,
            $this->pathFactory,
        ...
        $no_unsupported_links = new NoUnsupportedLinksExist(
            $environment,
            $translatable_factory,
            $no_abs_sym,
            $no_hard_links,
        ...
        $beginner_precondition = new BeginnerPreconditions(
            $environment,
            $common_preconditions,
            $no_unsupported_links,
            $staging_not_exists,
            $translatable_factory
        );
        $this->beginner = new Beginner($file_syncer, $beginner_precondition);
        ...
        $staging_dir_is_ready = new StagingDirIsReady(
            $environment,
            $translatable_factory,
            $staging_dir_exists,
            $staging_dir_writable
        );
        $cleaner_preconditions = new CleanerPreconditions(
            $environment,
            $common_preconditions,
   ...
   */
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln(
            '<info>Found an existing settings.php file and will append to it, to install a new site based on it</info>'
        );
    ...
    private function archiveAndPlace(
        OutputInterface $output,
        PathInterface $staging_dir,
        DrupalFinderComposerRuntime $finder
    ) {
    ...
    public function applyPatches(OutputInterface $output, string $drupal_root_in_stage)
    {
        $process = Process::fromShellCommandline(
            'patch -p1 < ' . __DIR__ . '/../patches/renderer-remove-fibers.patch',
            $drupal_root_in_stage
        );
        $process->run();
